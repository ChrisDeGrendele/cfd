...
while (t < t_finish) and timestepNum < Nt:
    ...
    if makeMovie:
        ...
        axs[0].plot(grid_sedas.x, grid_sedas.grid[RHOCOMP, :],'+', label='FD-Prim SEDAS', c='blue')
        ...
        axs[1].plot(grid_sedas.x, grid_sedas.grid[UCOMP, :],'+', label='FD-Prim SEDAS', c='blue')
        ...
        axs[2].plot(grid_sedas.x, grid_sedas.grid[PCOMP, :],'+', label='FD-Prim SEDAS', c='blue')
        ...
        for i in range(1, len(grid_sedas.x)-1):
            divV.append((grid_sedas.grid[UCOMP, i+1] - grid_sedas.grid[UCOMP, i-1])/(2*grid_sedas.dx))

        divV = np.array(divV)
        divV /= np.max(np.abs(divV))

        axs[3].plot(grid_weno.x, grid_weno.shock_mask, 'o', label='FD-Prim WENO',c='red')
        axs[3].plot(grid_sedas.x, grid_sedas.shock_mask,'+', label='FD-Prim SEDAS', c='blue')
        axs[3].plot(grid_sedas.x[1:len(grid_sedas.x)-1], divV, label="Divergence of Velocity")
