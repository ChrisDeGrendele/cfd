...
while (t < t_finish) and timestepNum < Nt:
    ...
    mass,mom,energy = prim_to_cons_var2D(primPX[RHOCOMP], primPX[UCOMP], primPX[VCOMP], primPX[PCOMP])
    consPX = np.array([mass,mom,energy])

    mass,mom,energy = prim_to_cons_var2D(primPY[RHOCOMP], primPY[UCOMP], primPY[VCOMP], primPY[PCOMP])
    ...
    for i in range(1, len(grid.x)-1):
        for j in range(1, len(grid.y)-1):
            ...
            for icomp in range(NUMQ):
                ...
                LFFlux[icomp, i] = 0.5*(fRX[icomp,i+1,j] + fRX[icomp,i,j] + fRY[icomp,i,j+1] + fRY[icomp,i,j] ) - 0.5*max_speed*(consP[icomp,i+1] - consP[icomp,i])
