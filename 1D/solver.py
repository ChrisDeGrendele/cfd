    ...
    while t < t_finish:
        ...
        for icomp in range(NUMQ):
            for i in range(grid.Nghost-1, grid.Nx + grid.Nghost):
                ...
                max_gradients[i] = max(max_gradients[i], gradient)  # Keep the maximum gradient across all variables

        # Determine the mask based on the maximum gradients
        threshold = np.mean(max_gradients)  # Define a suitable threshold
        shock_mask = max_gradients > 3*threshold  # True for non-smooth flow, False for smooth flow
        ...
        for icomp in range(NUMQ):
            ...
            gp_SE = GP_recipe1D(single_comp_grid, r_gp, ell = 24*grid.dx, stencil_method ="center", high_precision=True)
            gp_DAS = GP_recipe1D(single_comp_grid, 1, stencil_method="center", high_precision=True)
        ...
        #Compute SE Flux
        for icomp in range(NUMQ):
            grid.grid[icomp][grid.Nghost-1:grid.Nx+grid.Nghost+1] = SE_predict_by_comp[icomp]

        FSE = flux(grid)

        #Compute DAS Flux
        for icomp in range(NUMQ):
            grid.grid[icomp][grid.Nghost-1:grid.Nx+grid.Nghost+1]  = DAS_predict_by_comp[icomp]
        ...
        for icomp in range(NUMQ):
            ...
            for i in range(grid.Nghost, grid.Nx+grid.Nghost):
                ...
                if shock_mask[i]:
                    U_new[icomp, i] = U[icomp, i] - (dt / grid.dx) * (FDAS[icomp, i] - FDAS[icomp, i-1])

                else:
                    U_new[icomp, i] = U[icomp, i] - (dt / grid.dx) * (FSE[icomp, i] - FSE[icomp, i-1])
    ...
    while t < t_finish:
        ...
        for icomp in range(NUMQ):
            for i in range(grid.Nghost, grid.Nx+grid.Nghost):
                ...
                #LF
                U_new[icomp, i] = 0.5*(U[icomp,i+1] + U[icomp,i-1]) - (dt / (2 * grid.dx)) * (F[icomp, i+1] - F[icomp,i-1])
    ...
    while (t < t_finish) and timestepNum < nt:
        ...
        for i in range(grid.Nghost, grid.Nx + grid.Nghost - 1):
            for icomp in range(NUMQ):
                ...
                LFFlux[icomp, i] = 0.5*(fR[icomp,i] + fR[icomp,i+1]) - 0.5*max_speed*(consP[icomp,i+1] - consP[icomp,i])
    ...
    while (t < t_finish) and timestepNum < nt:
        ...
        for icomp in range(NUMQ):
            for i in range(grid.Nghost, grid.Nx + grid.Nghost):
                ...
                U_new[icomp, i] = 0.5 * (WL + WR)  # Updating the primitive variables with limited values
