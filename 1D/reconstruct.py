...
def SEDAS_apriori(U, grid):
    ...
    for icomp in range(NUMQ):
        ...
        gp_SE = GP_recipe1D(compGrid, r_gp, ell = 12*grid.dx, stencil_method ="center", high_precision=True)
        ...
        shockIndex = 0
        for i in range(grid.Nghost-1, grid.Nx+grid.Nghost+1):
            Uphalf[icomp,i] = SE_predict[smoothIndex] if (grid.shock_mask[i] == 0) else DAS_predict[shockIndex]
            if grid.shock_mask[i] == 0:
                smoothIndex += 1
            else:
                shockIndex += 1
