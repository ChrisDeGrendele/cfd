...
x = np.linspace(0, 2, Nx+2*NumGhost+1)
dx = x[1]-x[0]
...
DAS_v2_predict = gp_DAS_v2.convert_custom(x_predict, kern.DAS_v2, kern.DAS_v2)
DAS_v3_predict = gp_DAS_v3.convert_custom(x_predict, kern.DAS_v3, kern.DAS_v3)



plt.scatter(x, f,s=1.5, label='Data')
plt.plot(x+dx/2, weno5_interp(f, Nx, NumGhost),'-o', label='Weno5')
plt.plot(x_predict, DAS_predict, '-o',label='DAS')
plt.plot(x_predict, DAS_v2_predict, '-o',label='DAS V2')
plt.plot(x_predict, DAS_v3_predict, '-o',label='DAS V3')
