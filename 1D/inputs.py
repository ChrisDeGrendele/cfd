...
class CaseInsensitiveConfigParser(configparser.ConfigParser):
    def __init__(self, *args, **kwargs):
        ...
        self.optionxform = str.lower  # Transform options to lowercase for case insensitivity
...
class Inputs:
    ...
    def __init__(self, fname):
        ...
        #Time 
        self.time_steps = self.get_config_value(config, 'Time', 'time_steps', type_func=int, mandatory=False, default=np.inf)
        self.method = self.get_config_value(config, 'Time', 'method')
        self.t0 = self.get_config_value(config, 'Time', 't0', mandatory=False, default = 0.0,type_func=float)
        self.t_finish = self.get_config_value(config, 'Time', 't_finish', mandatory=False, default=np.inf,type_func=float)

        
        if (self.t_finish == np.inf) and (self.time_steps == np.inf):
            raise RuntimeError("Must supply either t_finish or time_steps in [Time] section of inputs file.")
        ...
        self.output_dir = self.get_config_value(config, 'Output', 'output_dir', mandatory=False, default="output/")
        self.make_movie = self.get_config_value(config, 'Output', 'make_movie', type_func=bool, mandatory=False, default=True)
    ...
    def get_config_value(self,config, section, option, type_func=str, mandatory=True, default=None):
        ...
        try:
            ...
            value = config.get(section, option, fallback=default if not mandatory else None)
            if value is None and mandatory:
                raise ValueError(f"Missing mandatory argument: '{option}' in section '{section}'")
            return type_func(value)
        except (configparser.NoSectionError, configparser.NoOptionError) as e:
            if mandatory:
                raise ValueError(f"Missing mandatory argument: '{option}' in section '{section}'") from e
            return default
        except ValueError as e:
            raise ValueError(f"Type conversion error for '{option}' in section '{section}': {e}") from e
