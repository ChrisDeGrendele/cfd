...
def test_missing_mandatory_option():
    with patch(
        "configparser.ConfigParser.get", side_effect=NoOptionError("nx", "Mesh")
    ) as mock_get:
        with pytest.raises(ValueError) as excinfo:
            inputs = Inputs(INI_FILE_PATH)
        assert "Missing mandatory argument: 'nx' in section 'Mesh'" in str(
            excinfo.value
        )
...
def test_type_conversion_error():
    with patch(
        "configparser.ConfigParser.get", return_value="not_a_number"
    ) as mock_get:
        with pytest.raises(ValueError) as excinfo:
            inputs = Inputs(INI_FILE_PATH)
